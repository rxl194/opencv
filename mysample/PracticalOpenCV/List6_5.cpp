...
class RANSACparams {
    ...
    public:
        RANSACparams(int _iter, int _min_inliers, float _dist_thresh, int _N) { //constructor
            iter_ = _iter;
            min_inliers_ = _min_inliers;
            dist_thresh_ = _dist_thresh;
            N_ = _N;
        }

        int get_iter() {return iter_;}
        int get_min_inliers() {return min_inliers_;}
        float get_dist_thresh() {return dist_thresh_;}
        int get_N() {return N_;}
};
...
class ellipseFinder {
    private:
        Mat img_; // input image
        vector<vector<Point> > contours_; // contours in image
        Mat Q_; // Matrix representing conic section of detected ellipse
    ...
    public:
        ellipseFinder(Mat _img, int l_canny, int h_canny, RANSACparams rp) { // constructor
            ...
            Mat edges; Canny(img_, edges, l_canny, h_canny);
            ...
            for(unsigned int i = 0; i < c.size(); i++) {
                ...
                if(d > 50) contours_.push_back(_c);
            }

            iter_ = rp.get_iter();
            min_inliers_ = rp.get_min_inliers();
            N_ = rp.get_N();
            dist_thresh_ = rp.get_dist_thresh();

            Q_ = Mat::eye(6, 1, CV_32F);
            ...
            //imshow("Edges", edges);
            */
            cout << "No. of Contours = " << contours_.size() << endl;
        }
...
bool ellipseFinder::is_good_ellipse(Mat Q) {
    ...
    if(minor_axis < thresh*major_axis || num/den1 < 0.f || num/den2 < 0.f || major_axis > max(img_.rows, img_.cols)) return false;
...
Mat ellipseFinder::RANSACellipse(vector<vector<Point> > contours) {
    ...
    for(int i = 0; i < contours.size(); i++) {
        ...
        if(c.size() < min_inliers_) continue;
        ...
        for(int j = 0; j < iter_; j++) {
            ...
            for(int k = 0; k < d.size(); k++)
                if(abs(d[k]) < dist_thresh_) consensus_set.push_back(rest[k]);
            // ...and find the random set with the most number of inliers
            if(consensus_set.size() > min_inliers_ && consensus_set.size() > best_inlier_score) {
...
vector<Point> ellipseFinder::ellipse_contour(Mat Q) {
    ...
    for(int i = 0; i < N; i++, theta += 2*PI/N) {
        ...
        Point p(x, y);
        if(x < img_.cols && y < img_.rows) ellipse.push_back(p);
    }
...
void ellipseFinder::debug() {
    ...
    cout << "No. of points in contour " << contours_[i].size() << endl;
    Mat a = fit_ellipse(contours_[i]);
    Mat img_show = img_.clone();
    drawContours(img_show, contours_, i, Scalar(0, 0, 255), 3);
    imshow("Debug contour", img_show);
    draw_inliers(a, contours_[i]);
    draw_ellipse(a);
}
...
void ellipseFinder::draw_inliers(Mat Q, vector<Point> c) {
    ...
    drawContours(img_show, contours_, -1, Scalar(0, 0, 255));
