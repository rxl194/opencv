...
typedef map<string, CvSVM> String_CvSVM_Map;

static CvSVM* get_CvSVM_Category (
        String_CvSVM_Map& svms,
        const string& category)
{
  String_CvSVM_Map::iterator mapIt = svms.find (category);
  return (mapIt != svms.end ()) ? &(mapIt->second) : 0;
}
...
void categorizer::train_classifiers ()
{
  ...
  for (int i = 0; i < categories; i++) {
    ...
    // Train SVM!
    CvSVM *svm = get_CvSVM_Category (svms, category);
...
void categorizer::categorize (VideoCapture cap)
{
  ...
  while (char(waitKey (1)) != 'q') {
    ...
    for (int i = 0; i < categories; i++) {
      string category = category_names[i];
      CvSVM *svm = get_CvSVM_Category (svms, category);
      float prediction = svm->predict (test, true);
