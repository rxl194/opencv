...
{
  im = imread (IMAGE_FOLDER + string ("/image.jpg"));
  int rotation_degrees = 30;
  Mat _M = getRotationMatrix2D (Point (im.cols / 2, im.rows / 2), rotation_degrees, 1);
  cout << _M << endl;

  warpAffine (im, im_transformed, _M, im.size (), INTER_LINEAR);
}

Point2f affine_transformer::get_click (string window_name, Mat _im)
{
  Point2f p (-1, -1);
